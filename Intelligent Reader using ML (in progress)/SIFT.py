...
Name = ['quad','quad1','double','double1','oct','oct2']  #change the folders here to learn the different note
Database = Data_create(Name)

Name = ['Test']                             #Test note should put in the TEST folder
...
for num,i in enumerate(DB_processed):
        ...
        for j in range(len(DB_processed[i])):
            ...
            new = cv2.resize(gray,(gray.shape[1]*4,gray.shape[0]*4),interpolation = cv2.INTER_CUBIC)
            new[new>=1]=255
            List.append(new)
        DB_1[i]=List
